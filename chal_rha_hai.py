...
def send_email_with_invoice(cart, total_price, customer_email):
    try:
        # Create PDF
        pdf = FPDF()
        pdf.add_page()
...
def move_to_payment(root, email_entry, total_label, send_receipt_button):
    global cart
    ...
    if confirmation:
        ...
        send_email_with_invoice(cart, total_price, customer_email)
        
        # Optionally, update GUI elements before closing
        messagebox.showinfo("Payment Success", "Payment successful!")
        
        # Enable send receipt button after successful payment
        send_receipt_button.config(state=tk.NORMAL)
    else:
    ...
    # Optionally, perform cleanup or other actions here
    
    # Example cleanup: clear the cart and update total label
    cart.clear()
    update_total_label(total_label)

def send_receipt_email(root, email_entry):
    customer_email = email_entry.get()
    send_email_with_invoice(cart, sum([item["price"] for item in cart]), customer_email)
    messagebox.showinfo("Receipt Sent", "Receipt sent successfully!")
...
def main():
    ...
    payment_button = tk.Button(frame, text="Proceed to Payment", command=lambda: move_to_payment(root, email_entry, total_label, send_receipt_button))
    payment_button.pack()

    send_receipt_button = tk.Button(frame, text="Send Receipt", command=lambda: send_receipt_email(root, email_entry), state=tk.DISABLED)
    send_receipt_button.pack()
